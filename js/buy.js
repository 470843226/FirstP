...
function createSelect(option){
	...
	select.onclick = function(){
		...
		select.className = ["select ", ["current", "normal"][status]].join("");
		status ^= 1;
	};
...
function createLi(option, num){
	var a = 0;
	var ul = document.createElement("ul");
	while(a < num){
		var oLi = document.createElement("li");
			oLi.appendChild(createAnchor(option[a].name, option[a].href));
			ul.appendChild(oLi);
			a++;	
	}
	return ul;
...
document.getElementById('brand').appendChild(createLi(data, data.length));